...
class PluginTest {
  private val String.fixtureDir: File
    get() = File("src/test/fixtures/$this")
  ...
  private fun testing(
    ...
    val runner =
      GradleRunner.create().withProjectDir(fixtureDir).withPluginClasspath().forwardOutput()

    if (clean) {
      runner.withArguments("clean").build()
    }

    val firstRun = runner.withArguments(*tasks, "--stacktrace", "--configuration-cache").build()
    ...
    val secondCachedRun =
      runner.withArguments(*tasks, "--stacktrace", "--configuration-cache").build()
  ...
  @Test
  fun `unchanged grammar files are up to date`() {
    ...
    val originalBar =
      bnfFile.copyTo(
        File(bnfFile.parent, "${bnfFile.nameWithoutExtension}.oldbnf"),
        overwrite = true,
      )
    ...
    assertEquals(
      TaskOutcome.SUCCESS,
      firstRun.task(":createComposablecom_example_barGrammar")?.outcome,
    )
    assertEquals(TaskOutcome.SUCCESS, firstRun.task(":generatecom_example_barParser")?.outcome)
    assertEquals(
      TaskOutcome.UP_TO_DATE,
      firstRun.task(":createComposablecom_example_fooGrammar")?.outcome,
    )
