...
abstract class BnfExtenderTask : DefaultTask() {
  ...
  @get:Internal abstract val root: Property<String>
...
internal abstract class ComposeGrammar : WorkAction<Outputs> {
  override fun execute() {
    ...
    GrammarFile(file = outputs.inputFile.asFile.get(), outputs = outputs)
      .generateComposableGrammarFile()
  }
}

private class GrammarFile(private val file: File, private val outputs: Outputs) {
  ...
  fun generateComposableGrammarFile() {
    ...
    val privateRules =
      rules.keys.filter { it.startsWith("private") }.map { it.substringAfter("private ") }
    ...
    val keyFinder =
      Regex(
        "([^a-zA-Z_]|^)(${unextendableSubclasses(header, rules.keys).joinToString("|")})([^a-zA-Z_]|$)"
      )
    val unextendableRuleDefinitions =
      rules
        .filterKeys { it in unextendableRules }
        .map {
  ...
  private fun generateRules(
    ...
    val keyFinder =
      Regex("([^a-zA-Z_]|^)(${(rules.keys + privateRules).joinToString("|")})([^a-zA-Z_0-9]|$)")

    val builder =
      StringBuilder("root ::= ${firstRule.extensionReplacements(keyFinder, privateRules)}\n")
  ...
  private fun generateRule(
    ...
    val definition =
      definition.replace(Regex("\\{([a-zA-Z_]*)}")) {
  ...
  private fun String.extensionReplacements(
    keysRegex: Regex,
    privateRules: Collection<String>,
  ): String {
    fun String.matcher() =
      replace(keysRegex) { match ->
  ...
  private fun String.toCustomFunction(): String {
    return replace(snakeCaseRegex) { matchResult ->
      matchResult.value.trim('_').replaceFirstChar { it.titlecase() }
    }
  }
  ...
  private fun unextendableRules(headerText: String, rules: Collection<String>): Sequence<String> {
    ...
    return keyFinder.findAll(headerText).map { it.groupValues[2] }.asSequence() +
      rules.filter { it.startsWith("private ") }
  }

  private fun unextendableSubclasses(
    headerText: String,
  ...
  private fun File.parserUtilName() =
    "${nameWithoutExtension.replaceFirstChar { it.titlecase() }}ParserUtil"

  private fun File.elementTypeHolderName() =
    "${nameWithoutExtension.replaceFirstChar { it.titlecase() }}Types"
  ...
  private fun generateParserUtil(
    ...
    val parserType =
      ClassName("com.intellij.lang.parser", "GeneratedParserUtilBase").nestedClass("Parser")
          ...
          .apply {
            rules.forEach { (key, definition) ->
              ...
              overrides?.let { overrides ->
                overrideFinder.matchEntire(definition)?.groupValues?.getOrNull(1)?.let {
                  if (it == "true") {
